...
@pytest.mark.parametrize("match_df",
                         [(Row(data="1"), Row(data="1"), Row(data="1"), Row(data="1"), Row(data="1")),
                          (Row(data="1"), Row(data="1"), Row(data="1"), Row(data="1"), Row(data="1"), Row(data="1"))])
def test_get_feature_set(spark_session_test: SparkSession, match_df) -> None:
    """Test get_feature_set."""
    match_df = extract_file(spark_session_test, "./data", "match.csv")
    player_attributes_df = extract_file(
        spark_session_test, "./data", "player_attributes.csv"
    )
    player_df = extract_file(spark_session_test, "./data", "player.csv")
    features_df = get_feature_set(match_df, player_attributes_df)
    assert features_df.count() == 2000
    assert sorted(features_df.columns) == sorted(
        [
            "win_ratio",
            "average_potential",
...
def test_get_player_stats(spark_session_test: SparkSession) -> None:
    ...
    out_df = get_player_stats(
        match_player_df, player_attributes_df, high_potential_threshold=87
    )
    assert sorted(out_df.collect()) == sorted(
        [
            Row(
